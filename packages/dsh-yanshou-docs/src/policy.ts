...
export function normalize(p: string, base: string): string {
...
/** 命中受保护目录时的说明文本 */
export function explain(target: string, config: Config): string {
  const abs = isAbsolute(target) ? target : resolve(config.installRoot, target)
  const rel = relative(config.installRoot, abs)
  return [
