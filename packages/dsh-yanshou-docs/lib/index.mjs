...
function registerTools(ctx, config) {
	...
	ctx.tools.register(defineTool({
		...
		description: "识别文档中的清单型表格，按表头列名匹配后接管数据行：数据不足则克隆行，多余则删除行。模板侧不含任何子表标记，识别完全靠表头列名序列 —— 所以不要改模板表头文字。P3 阶段引擎仍是 P0 骨架（P5 才做行克隆）；调用会得到明确的 not-implemented 摘要，而不是「脚本不存在」。",
		parameters: {
			docPath: {
				type: "string",
				required: true,
