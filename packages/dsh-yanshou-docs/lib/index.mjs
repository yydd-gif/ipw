...
function registerTools(ctx, config) {
	...
	ctx.tools.register(defineTool({
		name: "yanshou_docgen",
		description: "按目录清单生成文档：有模板的套模板，无模板的按 mode 三选一（template 选一个备用模板 / blank 建空白 / upload 占位等用户传 / skip 跳过）。itemId=all 只生成必选（收录=必选）；可选默认 0 份，须传具体 itemId。默认跳过已存在的文档，保护用户的编辑成果。确定性批处理，不调用模型推理。",
		parameters: {
			itemId: {
				type: "string",
				required: true,
